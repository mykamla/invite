...
class EventController {
  ...
  Future<void> addEvent({required context, required String nom, required String description, required String videoPath}) async {

    var eventState = Provider.of<EventState>(context, listen: false);

    String nameVideoUploading = videoPath + DateTime.now().toString();
    eventState.nameVideoUploading = nameVideoUploading;
    Reference ref = storage.ref().child(nameVideoUploading);
            ...
            await events.add(
                {
                  ...
                  'date_fin': DateTime.now().add(const Duration(days: 1)),
                  'vue_max': null,
