                ...
                child: TextField(
                 ...
                 minLines: widget.searchBoxMinLines,
                  decoration: widget.searchBoxDecoration?.copyWith(
                      hintText: widget.searchHint ?? widget.searchBoxDecoration!.hintText) ?? InputDecoration(hintText: widget.searchHint ?? "Find",
          ...
          Expanded(
            child: StreamBuilder<List<T>?>(
              ...
              //    return widget.errorBuilder?.call(context, snapshot.error) ?? Center(child: Text("Oops. \n${snapshot.error}"));
                  return widget.errorBuilder?.call(context, snapshot.error) ?? Center(child: Text(""));
                } else if (!snapshot.hasData) {
                  return widget.loadingBuilder?.call(context) ?? Center(child: Loading());
                } else if (snapshot.data!.isEmpty) {
                  return widget.emptyBuilder?.call(context) ?? Center(child: Text("Aucune playlist"));
                }
                return Scrollbar(
